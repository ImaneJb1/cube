...
/*   Updated: 2025/11/10 15:28:00 by ijoubair         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_texture	*init_text_arr(void *mlx_ptr, t_texture **arr, int size)
{
	...
	i = 0;
	*arr = gc_calloc(size, sizeof(t_texture) * size);
...
int	render_door(t_data *data, double x, double y, char c)
{
	...
	if (c == 'h')
	{
		if (data->ray.is_down && data->map[grid_y][grid_x] == 'D')
			return (1);
		else if (!data->ray.is_down && grid_y > 0
			&& data->map[grid_y - 1][grid_x] == 'D')
			return (1);
	}
	else if (c == 'v')
	{
		if (data->ray.is_right && data->map[grid_y][grid_x] == 'D')
			return (1);
		else if (!data->ray.is_right && grid_x > 0
			&& data->map[grid_y][grid_x - 1] == 'D')
			return (1);
	}
	return (0);
}
